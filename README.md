...
To verify requests against your own authentication system, pass a `getSessionToken` callback when initializing the SDK. The SDK calls it before every request to get a fresh token, which is sent as a `session-token` metadata header.
